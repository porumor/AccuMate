...
async function verifyAndSaveKey() {
    ...
    if (isValid) {
        isApiConnected = true;
        if (apiTimeout) clearTimeout(apiTimeout); // หยุดตัวนับเวลา 10 วิทันทีที่เชื่อมต่อสำเร็จ
...
function copyInputFieldText() {
    const inputEl = document.getElementById('userInput');
    const iconEl = document.getElementById('copyInputIcon');
